...
public class MathUtils
{
    ...
    public static BigDecimal round(final float f, final int newScale) {
        return new BigDecimal(Float.toString(f)).setScale(newScale, 4);
    }
    ...
    public static void drawUnfilledCircle(final float n, final float n2, final float n3, final float n4, final int n5) {
        ...
        int n10 = 0;
        if (n10 <= 360) {
            GL11.glVertex2d(n + Math.sin(n10 * 3.141592653589793 / 180.0) * n3, n2 + Math.cos(n10 * 3.141592653589793 / 180.0) * n3);
            ++n10;
            return;
        }
        GL11.glEnd();
